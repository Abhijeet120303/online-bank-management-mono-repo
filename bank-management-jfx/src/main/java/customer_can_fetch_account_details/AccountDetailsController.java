...
public class AccountDetailsController {
  ...
  @FXML private TextField registerDate;
  ...
  @FXML private Button fetchDetails;
  ...
  private void fetchDetails() throws IOException {
    try {
      // Long customerIdValue = Long.parseLong(customerId.getText()); // Assuming the customer id is
      // entered as a number

      String apiUrl =
          "http://localhost:8081/api/v1/customer/get/information/" + customerId.getText();

      GetCustomerInformationResponse response =
          RestUtil.sendGetRequest(apiUrl, GetCustomerInformationResponse.class);

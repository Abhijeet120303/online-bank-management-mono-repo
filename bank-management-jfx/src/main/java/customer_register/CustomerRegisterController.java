...
public class CustomerRegisterController {
  ...
  @FXML private ComboBox<String> selectBranch;

  @FXML private TextField password;

  @FXML private TextField permanentAddress;
  ...
  public void initialize() {
    ...
    ObservableList<String> genderOptions =
        FXCollections.observableArrayList("Male", "Female", "Others");
  ...
  private void fetchBranchOptions() {
    try {
      ...
      List<String> branchNames =
          RestUtil.sendGetRequests(
              "http://localhost:8081/api/admin/get/branchNames", typeReference);
  ...
  public void register(ActionEvent event) {
    ...
    if (firstNameText.isEmpty()
        ...
        || emailText.isEmpty()
        || mobileNumberText.isEmpty()
        || genderValue == null
        || branchValue == null
        || passwordText.isEmpty()
        || permanentAddressText.isEmpty()
    ...
    try {
      ...
      if ("Success".equals(response.getStatus())) {
        ...
        showAlert(
            "Success",
            "Customer Id for register Customer is as :" + response.getData().getCustomerId());
      } else {

...
public class UpdateDeleteSearchCustomerScreenController {
  ...
  @FXML private TextField presentAddress;

  @FXML private TextField password;

  @FXML private TextField accountStatus;
  ...
  private void searchUser() throws InterruptedException {
    try {
      String searchUrl =
          "http://localhost:8081/api/v1/admin/search/customer/" + customerId.getText();

      AdminCanSearchTheCustomersResponse response =
          RestUtil.sendGetRequest(searchUrl, AdminCanSearchTheCustomersResponse.class);
  ...
  private void editUser() throws InterruptedException {
    ...
    try {
      ...
      String updateUrl =
          "http://localhost:8081/auth/v1/admin/update/customer/" + customerId.getText();

      AdminCanEditTheCustomersResponse response =
          RestUtil.sendPostRequest(updateUrl, AdminCanEditTheCustomersResponse.class, table);
  ...
  private void delete() {
    try {
      String deleteUrl =
          "http://localhost:8081/api/v1/admin/delete/customer/" + customerId.getText();

      AdminCanDeleteTheCustomersResponse status =
          RestUtil.sendDeleteRequest(deleteUrl, AdminCanDeleteTheCustomersResponse.class);
